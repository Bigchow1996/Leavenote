...
layui.use('laydate', function(){
	...
	function confirmDel(del_pass,del_time,del_pageNum,del_pageSize,del_noteid,del_studentid)
	{	
	    if(confirm("确定删除吗？"))
	    {
	         var studentid = del_studentid;
      		 var noteid =del_noteid;
       		 var pass =del_pass;
       		 var time =del_time;
       		 var pageSize =del_pageSize;
       		 var pageNum =del_pageNum;
	         $.post("delete", {	
				studentid:studentid,
				noteid:noteid,
				time:time.replace(/[^\d]/g,'-')	,
...
<body>
	...
	<div class="divselect">
			...
			<input type="text" id="Cname" class="form-control"  placeholder="按请假开始时间查询" style="cursor:pointer;"  readonly="readonly"/>
			</div>
			<div  class="divinline">
	...
	<br>
	<div class="insertdiv" style="margin-top: 0;">
		<div id="divlist">
			<c:forEach items="${pageUtil.list}" var = "each" varStatus="vars" >
					<c:choose>
    					<c:when test="${each.pass=='同意'}" ><a href="#"  class="mya"></c:when>
    					<c:when test="${each.pass=='不同意'}" ><a href="#"  class="mya"></c:when>
    					<c:otherwise><a href="shownote?noteid=${each.id}" class="mya"></c:otherwise>
    				</c:choose>
    				<c:choose>
    					<c:when test="${each.pass=='同意'}" ><div class="showlist_pass"></c:when>
    					<c:when test="${each.pass=='不同意'}" ><div class="showlist_unpass"></c:when>
    					<c:otherwise><div class="showlist_passing"></c:otherwise>
    				</c:choose>
	    			<div class="left">
	                 ...
	                 <c:choose>
    					...
    					<c:otherwise><font style="color:orange"><b>待审批</b>&nbsp;可以修改</font>&nbsp;&nbsp;&nbsp;<a  onclick="confirmDel('${pass}','${time }','${ pageUtil.pageNum}','${ pageUtil.pageSize}','${each.id }','${student.id }')"  style="cursor:pointer;">删除</a></c:otherwise>
    				</c:choose> 
