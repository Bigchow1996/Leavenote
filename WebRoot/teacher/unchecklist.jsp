        ...
        <tr align="center">
            ...
            <%
		 	...
		 	<td class="<%=my_td_style %>"><!-- 打印列 -->
                <div align="center" class="my_text_11">
                    <div id="<%="pr"+j%>" style="display:none">
...
<script type="text/javascript">
    function checkpass(passstatus, notesid, radioid) {
        ...
        if (passstatus == "同意") {
                ...
                xmlhttp.onreadystatechange = function () {
                    if (xmlhttp.readyState == 4) {// 4 = "loaded"
                        if (xmlhttp.status == 200) {// 200 = "OK"
                            ...
                            document.getElementById("pr"+radioid).style.display = "block";
                        } else {
            ...
            var unpassreason = window.prompt("您拒绝了该同学的请假申请,请给出理由:", "输入拒绝理由(不能为空且长度小于20)");
            if (unpassreason == "" || unpassreason == "输入拒绝理由(不能为空且长度小于20)") {     //判断拒绝理由是否为空
                window.alert("拒绝理由不能为空！");
                $("#radio_b" + radioid).removeAttr("checked");
            }else if(unpassreason.length > 20){
            	window.alert("字数不能超过20（包含符号）");
                $("#radio_b" + radioid).removeAttr("checked");
